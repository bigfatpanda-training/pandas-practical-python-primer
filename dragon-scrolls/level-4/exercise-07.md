...
- Previously, this was somewhat obscured by the ugly error. Now that we've added
  our test harness, let's see what we get back:
    ...
    - The keys to pick up from this error message is that the error occurred at 
    line 55 (your location may be slightly different depending on your program 
    line numbers) and that an attempt was made to access an element of a
    `NoneType` object - which is impossible.  You can only access elements of
    container objects like tuples, lists, sets, and dicts.
    - From this we can deduce that the `None` object is being passed around
    our code where a container object is required.
...
- Looking at the code at line 55, I see this statement:
    ...
    - There is only one object here on which I'm trying to elements of a
    container, the `request_payload` dict.  So that is the object from which 
    the error is being generated.  
    
    - So now the question is why the value assigned to that name `None`?  Well, 
    where is that name assigned?  
...
is not `None`.  If it is, the user should be informed that the API requires
JSON payloads:
...
- From this, I can tell that the exception occurred at line 43 in my code and
